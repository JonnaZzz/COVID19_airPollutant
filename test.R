...
param0 <- list(
  ...
  , "min_child_weight" = 1
  , "max_depth" = 4 
  , 'monotone_constraints'=c(1,0,0,1,1,1,0,1,1)
)
...
for (i in df_list){
  ...
  df_train<-subset(df,year<2021)
  xgtrain<-xgb.DMatrix(as.matrix(df_train[,-c(2,3,6,11,12,15)]), 
                       label = df_train$weekly_death)
  xg<-xgb.DMatrix(as.matrix(df[,-c(2,3,6,11,12,15)]),
                  label = df$weekly_death)
  
  xgb<-xgb.cv(data = xgtrain,
              nrounds = 350, params = param0, nfold = 5, verbose=1)
  ...
  df_train<-df_h2o[df_h2o$year<2021,]
  fit<-h2o.gam(x=c('year','week','mean_temp','pm10','no2','o3','humidity',
                   'flu_a','flu_b'),y='death_rate',
               training_frame = df_train,nfolds = 5,seed=123,alpha = 0,
  ...
  plot_temp<-cbind(df[,c('idx','weekly_death')],pred_gam*df$pop,pred_xg) # get predicted death count
  ...
  for (j in variable_list){
    ...
    df_new<-df
    df_new[,j]<-0
    xg<-xgb.DMatrix(as.matrix(df_new[,-c(2,3,6,11,12,15)]) 
    )
  ...
  plot_attr[[which(df_list==i)]]<-tt%>%group_by(year,age_group,variable)%>%summarise(yearly_attri=sum(death_attr))%>%
  ...
  variable_attr<-rbind(variable_attr,tt)
}

year_week <- df_all %>% select(year,idx,week,start_date) %>% rename(week_number='idx')
predict_count <- merge(year_week,plot_df,by.x='week_number',by.y='week_number',all.y = T)
write.csv(predict_count,"result/weekly_predict.csv")
...
pl<-ggplot(data=plot_df,aes(x=week_number))+geom_line(aes(y=actual_death,color='Observed (Covid death excluded)'))+
  geom_line(data=subset(plot_df,week_number<=523),linetype=1,aes(y=predicted_xg,color='XGBoost'))+
  geom_line(data=subset(plot_df,week_number>=523),linetype=6 ,aes(y=predicted_xg,color='XGBoost'))+
  geom_line(data=subset(plot_df,week_number<=523),linetype=1,aes(y=predicted_gam,color='GAM'))+
  geom_line(data=subset(plot_df,week_number>=523),linetype=6 ,aes(y=predicted_gam,color='GAM'))+
  geom_vline(xintercept=575,linetype = 2)+
  scale_x_continuous(breaks = c(2, 53, 105, 158, 210, 262, 314,366,419,471, 523, 575),
...
ggsave("result/prediction.pdf", pl)
...
for (i in age_list){
  excess_temp<-data.frame(excess_death = rep(NA,2),
                          ...
                          age_group = rep(i,2),
                          method=c('gam','xg'))
  df<-plot_df%>%filter(week_number>523)%>%filter(age_group==i)%>%
  ...
  excess_temp$excess_death[1]<-ann[order(ann)][500]
  excess_temp$lwr[1]<-ann[order(ann)][25]
  excess_temp$upr[1]<-ann[order(ann)][975]
  ...
  excess_temp$excess_death[2]<-ann[order(ann)][500]
  excess_temp$lwr[2]<-ann[order(ann)][25]
  excess_temp$upr[2]<-ann[order(ann)][975]
...
annual_excess_1 <- plot_df%>%filter(week_number>523)%>%mutate(excess_gam=actual_death-predicted_gam,
...
write.csv(annual_excess,'result/annual_excess_all_cause_pandemic.csv',quote=F,row.names = FALSE)
...
pl<-ggplot(bar_plot_df,aes(x=variable,y=excess_rate,fill=category,group=category))+
...
ggsave("result/annual_excess.pdf", pl)
...
for (j in test_year){
  ...
  for (i in df_list){
    ...
    xgtrain<-xgb.DMatrix(as.matrix(df_train[,-c(2,3,6,11,12,15)]),
                         label = df_train$weekly_death)
    xgval<-xgb.DMatrix(as.matrix(df_val[,-c(2,3,6,11,12,15)]),
                       label = df_val$weekly_death)
    xg<-xgb.DMatrix(as.matrix(df[,-c(2,3,6,11,12,15)]),
                    label = df$weekly_death)
    
    xgb<-xgb.cv(data = xgtrain,
                nrounds = 50, params = param0,nfold = 5)
    nround<-xgb$evaluation_log$iter[which.min(xgb$evaluation_log$test_poisson_nloglik_mean)]
  ...
  print(seq(from=min(df$idx),to=max(df$idx)+1,by=52))
  sensitivity_plot[[j]]<-ggplot(data=plot_df,aes(x=week_number))+geom_line(aes(y=actual_death,color='real'))+
    ...
    scale_x_continuous(breaks = round(seq(from=min(df$idx),to=max(df$idx)+1,by=52)),
                       labels = as.character(seq(from=2009,to=j)))+
    labs(title=paste( 'Using',as.character(2009), 'to',as.character(j-1),
...
library(corrplot)
data_cor <- cor(df_train[,-c(3,6,11,12,15)])

corrplot(data_cor, method = "circle", type = "upper", tl.col = "black", tl.srt = 45)
